...
Future<Stream<Order>> getOrders() async {
  ...
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}orders?${_apiToken}with=user;productOrders;productOrders.product;orderStatus&search=user.id:${_user.id}&searchFields=user.id:=&orderBy=id&sortedBy=desc';
...
Future<Order> addOrder(Order order, Payment payment) async {
  try {
    ...
    if (prefs.getString('select_address_id') != null)
      params['delivery_address_id'] = prefs.getString('select_address_id');
    else if ([' ', '', null, '0', 'null']
        .contains(params['delivery_address_id']))
      params['delivery_address_id'] = null;
    // Helper.printToConsole(params['delivery_address_id']);
    ...
    Helper.printToConsole(order.toMap());
    await prefs.remove('select_address_id');

    Helper.printToConsole(response.body);
    Helper.printToConsole('-----------------------------------');
    Helper.printToConsole(response.statusCode);
