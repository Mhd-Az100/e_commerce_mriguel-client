...
import 'package:markets/src/elements/pills_widget.dart';
...
class _MarketCategoryWidgetState extends StateMVC<MarketCategoryWidget> {
  ...
  void initState() {
    scrollController.addListener(() {
      ...
      if (maxScroll - currentScroll <= delta) {
        if (_con.selectedCat == '-1')
          _con.listenForPaginateProductsByMarketCategory(
              id: widget.routeArgument.id,
              marketId: widget.routeArgument.marketID);
        else
          _con.listenForPaginateProductsByMarketSubCategory(
            id: widget.routeArgument.id,
            marketId: widget.routeArgument.marketID,
            subcat: _con.selectedCat,
          );
      }
    });
    _con.listenForCategory(
        id: widget.routeArgument.id, cat: widget.routeArgument.category);
    _con.listenForProductsByMarketCategory(
        id: widget.routeArgument.id, marketId: widget.routeArgument.marketID);
    _con.listenForCart();
    super.initState();
  }
                          ...
                          (index) => InkWell(
                            onTap: () {
                              if (_con.category.subCategories
                                ...
                                _con.listenForProductsByMarketSubCategoryForMarket(
